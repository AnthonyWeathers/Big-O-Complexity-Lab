...
// ANSWER: const findFourth = numbers => return numbers[3]
...
// ANSWER: str in both are the input array but changed to be all lower cased letters, making them O(n), the letters object in countLetters remains constant so doesn't affect the space complexity. Letters in countLettersRoundTwo increases space taken by letters, but it is less than str by a varying amount, so it is more of O(1) as it does not scale to str. There is a new variable called splitStr which is the same amount of space as the str string size, but that is only another O(n) making it O(2n) which boils down to O(n), therefore making both simply O(n)
